...
logdir=${logBasePath}/logInit

mkdir -p logdir

errorBasePath=${RootPathOut}/errors
errordir=${errorBasePath}/errorInit
...
for dataset in $datasets; do
        ...
        echo sbatch --job-name=${label} --output=${outfile}.out --error=${errorfile}.out -C${gpu} ${scriptdir}/Sbatch_test_Initialization.sh ${label} ${dataset} ${Outdir} ${dataPath} ${logdir} ${Imp} ${Alldata}
        RES=$(sbatch --job-name=${label} --output=${outfile}.out --error=${errorfile}.out -C${gpu} ${scriptdir}/Sbatch_test_Initialization.sh ${label} ${dataset} ${Outdir} ${dataPath} ${logdir} ${Imp} ${Alldata})
        echo ${RES}
